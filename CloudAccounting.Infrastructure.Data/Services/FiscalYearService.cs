...
public class FiscalYearService(IFiscalYearRepository repository) : IFiscalYearService
{
    private readonly IFiscalYearRepository _repository = repository;

    public async Task<Result<FiscalYear>> CreateFiscalYear(int companyCode, int fiscalYearNumber, DateTime startDate)
    {
        ...
        (
            ...
            await IsAlreadyAnInitialFiscalYear(companyCode),
            false,
            false,
            DateTime.MinValue,
            []
        ...
        Result<FiscalYear> result = await _repository.InsertFiscalYearAsync(fiscalYear);
    ...
    private async Task<bool> IsAlreadyAnInitialFiscalYear(int companyCode)
    {
        Result<bool> result = await _repository.DoesCompanyHaveInitialFiscalYear(companyCode);
